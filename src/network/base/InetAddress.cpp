...
:addr_(ip),port_(std::to_string(port)),is_v6_(is_v6){
...
void InetAddress::SetIsV6(bool is_v6){
    is_v6_ = is_v6;
}
...
uint32_t InetAddress::IPv4(const char* ip) const{
    ...
    if(inet_pton(AF_INET,ip,&addr_in.sin_addr) < 0){
        NETWORK_ERR<< "ipv4 ip:"<<ip<<" convert failed.";
    }
...
bool InetAddress::IsWanIp() const{
    ...
    return !IsLanIp() && ip!=INADDR_LOOPBACK;
}
